...
import CompletedInfoTodoItem from "../../components/reusable/TodoItem/CompletedInfoTodoItem";
...
const categorizedTodoListSelector = createSelector(
  ...
  (todoList: TodoType[]) => {
    ...
    todoList.forEach((todo) => {
      const doneAt = format(new Date(todo.doneAt!), "MMMM d, yyyy");
      const category = categorizedTodoList[doneAt] || [];
      categorizedTodoList[doneAt] = category.concat(todo);
    });
            ...
            <div key={categoryKey} className="my-10">
              ...
              <ul className="bg-paper my-1 rounded shadow p-2 divide-y-2">
                {categorizedTodoList[categoryKey].map((todo) => (
                  <li key={todo.id}>
                    <CompletedInfoTodoItem data={todo} />
                  </li>
                ))}
              </ul>
            </div>
